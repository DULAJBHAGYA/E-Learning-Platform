...
class CourseService {
  ...
  Future<dynamic> fetchAllCourses() async {
    try {
      ...
      final response = await _dio.get('/courses?page_id=1&page_size=100');
...
final response = await _dio.get('/courses/byuser?user_id=$user_id&page_id=1&page_size=100');
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Failed to fetch course by ID. Status code: ${response.statusCode}');
    }
  } on DioError catch (e) {
    print("Dio Error: $e");
    print("Response Data: ${e.response?.data}");
  ...
  enrollInCourse(int courseId, String username, String accessToken, String refreshToken) {}
}